...
const replaceHashInMatch = (grunt, match, prefix, index) => {
    const filename = grunt.file.expand({ cwd: 'build/web-audio-meetup-may-2019/scripts' }, `${ prefix }.*.js`)[0];
    const hash = `sha384-${ computeHashOfFile(`build/web-audio-meetup-may-2019/scripts/${ filename }`, 'sha384', 'base64') }`;
...
module.exports = (grunt) => {
    return {
        ...
        'csp-production': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: () => {
                        const html = fs.readFileSync('build/web-audio-meetup-may-2019/index.html', 'utf-8');
                        const regex = /<script[^>]*?>([^<](.|[\n\r])*?)<\/script>/gm;
                        const scriptHashes = [];
        ...
        'fix': {
            files: {
                './': [
                    'build/web-audio-meetup-may-2019/ngsw-worker.js'
                ]
            },
            options: {
                /*
        ...
        'manifest': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (_, filename, extension) => grunt.file.expand({ cwd: 'build/web-audio-meetup-may-2019' }, `assets/*.${ filename }.${ extension }`)[0]
                }, {
                    ...
                    match: /[\s]*"\/web-audio-meetup-may-2019(\/scripts)?\/runtime(?:-es(?:2015|5))?.[a-z0-9]*\.js",/g,
                    replacement: ''
                }, {
                    match: /[\s]*"\/web-audio-meetup-may-2019(\/scripts)?\/runtime(?:-es(?:2015|5))?.[a-z0-9]*\.js":\s"[a-z0-9]+",/g,
                    replacement: ''
                }, {
                    // Replace the hash value inside of the hashTable for "/scripts/main-es*.js" because it was modified before.
                    match: /"\/web-audio-meetup-may-2019(\/scripts\/main(?:-es(?:2015|5))?.[a-z0-9]+.js)":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-meetup-may-2019${ filename }": "${ computeHashOfFile(`build/web-audio-meetup-may-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/styles/styles*.css" because it was modified before.
                    match: /"\/web-audio-meetup-may-2019(\/styles\/styles\.[a-z0-9]*\.css)":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-meetup-may-2019${ filename }": "${ computeHashOfFile(`build/web-audio-meetup-may-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/web-audio-meetup-may-2019\/(index|start)\.html":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-meetup-may-2019/${ filename }.html": "${ computeHashOfFile(`build/web-audio-meetup-may-2019/${ filename }.html`, 'sha1', 'hex') }"`;
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute) => {
                        if (moduleAttribute === undefined) {
                            return `<script>${ fs.readFileSync(`build/web-audio-meetup-may-2019/${ filename }`) }</script>`;
                        }

                        return `<script${ moduleAttribute }>${ fs.readFileSync(`build/web-audio-meetup-may-2019/${ filename }`) }</script>`;
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute, initialHash) => {
                        const updatedHash = (/main(?:-es(?:2015|5))?.[a-z0-9]*\.js/.test(filename)) ?
                            `sha384-${ computeHashOfFile(`build/web-audio-meetup-may-2019/scripts/${ filename }`, 'sha384', 'base64') }` :
                            initialHash;
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (match, filename) => {
                        const hash = `sha384-${ computeHashOfFile(`build/web-audio-meetup-may-2019/styles/${ filename }`, 'sha384', 'base64') }`;
