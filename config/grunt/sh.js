    ...
    return {
        ...
        'lint-src': {
            cmd: `htmlhint --rules ${convertConfig(documentConfig)} 'src/**/index.html' && htmlhint --rules ${convertConfig(
                templateConfig
            )} 'src/app/**/*.component.html' && ng lint web-audio-meetup-may-2019 --type-check`
        },
        'lint-test': {
        ...
        'prerender': {
            cmd: 'ng run web-audio-meetup-may-2019:server:production && angular-prerender --preserve-index-html'
        },
        'preview': {
        ...
        'smoke': {
            cmd: env.TRAVIS
                ? "IS_SMOKE_TEST=true ng e2e --dev-server-target '' && hint --telemetry=off https://chrisguttandin.github.io/web-audio-meetup-may-2019"
                : "webdriver-manager update && IS_SMOKE_TEST=true ng e2e --dev-server-target '' --no-webdriver-update && hint --telemetry=off https://chrisguttandin.github.io/web-audio-meetup-may-2019"
        },
        'test': {
        ...
        'verify': {
            cmd:
                "bundle-buddy build/web-audio-meetup-may-2019/*.js.map && grep -r build/**/*.map -e '/environments/environment.ts'; test $? -eq 1"
        }
    };
};
