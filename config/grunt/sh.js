...
module.exports = {
    ...
    lint: {
        cmd: `htmlhint --rules ${ convertConfig(documentConfig) } 'src/**/index.html' && htmlhint --rules ${ convertConfig(templateConfig) } 'src/app/**/*.component.html' && ng lint web-audio-meetup-may-2019 --type-check && ng lint web-audio-meetup-may-2019 --configuration test`
    },
    monitor: {
        cmd: 'ng serve --aot'
    },
    ...
    verify: {
        cmd: "bundle-buddy build/web-audio-meetup-may-2019/*-es2015.*.js.map && bundle-buddy build/web-audio-meetup-may-2019/*-es5.*.js.map && grep -r build/**/*.map -e '/environments/environment.ts'; test $? -eq 1"
    }
};
